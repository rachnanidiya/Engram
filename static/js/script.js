...
/* ── EXPORT ACTIVE DECK DATA AS FILE ── */
function exportActiveDeck() {
    ...
    const sanitizedFileName = rawTitle.toLowerCase().replace(/[^a-z0-9]/gi, '_') + "_backup.txt";
...
/* ── PHASE 4: FUZZY KEYWORD MATCHING QUIZ EVALUATOR (FIXED ACCURACY) ── */
function evaluateQuizAnswer(event, cardId, rawCorrectAnswer) {
    ...
    // Helper: Tokenize text into descriptive alphabetic keyword sets, discarding filler articles
    const stopWords = new Set(["the", "a", "an", "is", "are", "was", "were", "of", "in", "on", "at", "by", "for", "with", "to", "and", "or", "that", "it", "from"]);
